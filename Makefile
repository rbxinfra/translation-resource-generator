...
build-release-x64: vendor
	...
	@cd ./src && export GOARCH=$(GOARCH)&& export GOOS=$(GOOS)&& export CGO_ENABLED=1&& go build -mod=vendor -tags enablecgo -ldflags "-s -w -X main.commitSha=$(IMAGE_TAG) -X main.buildMode=release -X main.applicationName=$(PROJECT_NAME)" -v -o ../bin/release/$(GOOS)/x64/$(OUTFILE) ./main.go $(TO_NUL) || echo Go Build returned a non-zero status code
